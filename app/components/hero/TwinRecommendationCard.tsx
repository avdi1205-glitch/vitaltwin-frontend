...
import { HERO_DATA_DISCLAIMER, RECOMMENDATIONS } from './heroData';
 ...
 * Clearly marked as an illustrative example (see HERO_DATA_DISCLAIMER); no
    ...
    <div className="mx-auto w-full max-w-4xl rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-sm md:p-8">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-[#B7BDC4]">Heute würde ich dir empfehlen:</p>

          <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
            {RECOMMENDATIONS.map((item) => (
              <li key={item.text} className="flex items-center gap-2.5 text-sm text-[#F5F2EA]">
      ...
      <p className="mt-5 border-t border-white/5 pt-4 text-xs text-[#8E969F]">{HERO_DATA_DISCLAIMER}</p>
    </div>
  );
}
