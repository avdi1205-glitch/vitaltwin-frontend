...
export default function FamilyGoalsSection({
  ...
  const t = useTranslations('familyGoals');
  ...
  return (
    <div id="family-goals-section" className="mt-6 border-t border-white/10 pt-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="font-[family-name:var(--font-serif-display)] text-lg font-semibold text-[#F5F2EA]">{t('title')}</h3>
        {role === 'owner' && (
          <button
          ...
          {goals.map((goal) => (
            <div key={goal.id} className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3">
              ...
              <p className="mt-2 text-xs text-[#8E969F]">
                {goal.participant_count} {t('participantsCount')} · {goal.completed_count} {t('completedCount')}
              </p>
